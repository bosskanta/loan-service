...
public class LoanControllerTest {
    ...
    void testGetLoanInfoByCustomerIdEquals1() throws Exception {
        ...
        assertEquals("1", data.get("id").toString());
        assertEquals("OK", data.get("status").toString());
        assertEquals("102-222-2200", data.get("account_payable").toString());
        assertEquals("102-333-2020", data.get("account_receivable").toString());
        assertEquals(90, data.get("principal_amount"));

        verify(loanService, times(1)).getLoanInfoById(reqParam);
    }

    @DisplayName("Test get loan info by customer id 2 should return Get loan info")
    @Test
